...
class ServiceModel extends Model {
    ...
    public function getById($id) {
        ...
        if ($service) {
            ...
            // Fetch Packages
            $stmt_p = $this->db->prepare("SELECT * FROM service_packages WHERE service_id = ?");
    ...
    public function syncPackages($service_id, $packages) {
        ...
        foreach ($packages as $p) {
            ...
            $stmt = $this->db->prepare("INSERT INTO service_packages (service_id, name, price, period, features, tagline, description, original_price, savings, color, recommended) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([
                $service_id,
                $p['name'],
                $p['price'],
                ...
                $p['savings'] ?? '',
                $p['color'] ?? '',
                isset($p['recommended']) ? (int)$p['recommended'] : 0
            ]);
        }
    }
